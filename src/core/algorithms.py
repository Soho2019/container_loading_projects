...
def calculate_volume_utilization(
    products: List[ProductsSpec], container: ContainerSpec, use_pallet: bool
) -> float:
    """
    ...
    for product in products:
        if product.length <= 0 or product.width <= 0 or product.height <= 0:
            raise ValueError(f"Product dimensions must be positive")

    total_volume = sum(
        product.length * product.width * product.height for product in products
    )
    if not use_pallet:
        # 不排拖情况，直接计算货物体积占比
        container_volume = container.length * container.width * container.height
        if container_volume <= 0:
            raise ZeroDivisionError("容器体积必须大于0")
        return total_volume / container_volume
    else:
        # 排拖情况，需扣除托盘高度（托盘高度统一为150mm）
        effective_container_height = max(
            container.height - BusinessRules.PALLET_HEIGHT, 0
        )
        if effective_container_height <= 0:
            return ValueError("使用托盘后容器有效高度小于等于0")

        # 利用率 = (货物体积 / 集装箱总体积(扣除托盘高度))
        effective_container_volume = (
            container.length * container.width * effective_container_height
        )
        if effective_container_volume <= 0:
            raise ZeroDivisionError("容器有效体积必须大于0")

        return total_volume / effective_container_volume


def calculate_weight_utilization(
    products: List[ProductsSpec], container: ContainerSpec
) -> float:
    """计算载重利用率"""
    ...
    if total_weight > container.max_weight:
        raise ValueError(
            f"载重超限: 总重量 {total_weight} 超过容器最大载重 {container.max_weight}"
        )
...
def calculate_center_offset(
    ...
    return (
        ...
        if return_total
        else (offset_x, offset_y, offset_z)
    )
...
def dominates(a: Solution, b: Solution) -> bool:
    ...
    return (
        better_volume
        and better_weight
        and better_stability
        and (
            a.volume_utilization > b.volume_utilization
            or a.weight_utilization > b.weight_utilization
...
class GeneticAlgorithmOptimizer:
    ...
    def optimize(self) -> Solution:
        ...
        for _ in range(self.max_gen):
            ...
            # 精英保留
            elites = [
                ind for ind, _ in evaluated[: int(self.elite_ratio * self.pop_size)]
            ]
        ...
        weight_utilization = (
            total_weight / self.container.max_weight
    ...
    def _generate_individual(self) -> List[PalletSpec]:
        ...
        while used_length < max_length:
            valid_pallets = [
                p
                for p in self.candidate_pallets
    ...
    def _fitness(self, individual: List[PalletSpec]) -> float:
        ...
        # 检查托盘布局是否有效
        if not self._is_valid_layout(individual):
            return 0.0  # 无效布局直接返回 0
    ...
    def _is_valid_layout(self, individual: List[PalletSpec]) -> bool:
        """检查托盘布局是否有效"""
        used_length = 0
        max_length = self.container.length - self.container.door_reserve

        for pallet in individual:
            if pallet.length > max_length - used_length:
                return False
            used_length += pallet.length + BusinessRules.PALLET_GAP["longitudinal"]
        return True
    ...
    def _calc_fill_ratio(self, pallets: List[PalletSpec]) -> float:
        ...
        for pallet in pallets:
            # 扣除托盘边缘的间隙
            eff_length = (
                pallet.length - 2 * BusinessRules.GAP_OF_GOODS_AND_THE_EDGE_OF_PALLET
            )
            eff_width = (
                pallet.width - 2 * BusinessRules.GAP_OF_GOODS_AND_THE_EDGE_OF_PALLET
            )
            ...
            filled_area = sum(
                ...
                if p.length <= eff_length and p.width <= eff_width
            )

            total_filled += min(filled_area, eff_length * eff_width)

        # 计算总托盘面积
        total_pallet_area = sum(p.length * p.width for p in pallets)
        if total_pallet_area <= 0:
            return 0.0

        return total_filled / total_pallet_area
    ...
    def _calculate_stability(
        self, pallets: List[PalletSpec], positions: List[Tuple[float, float, float]]
    ) -> float:
        """计算稳定性得分"""
        if not pallets:
            return 0.0
        ...
        weighted_x = sum(
            (pos[0] + p.length / 2) * p.max_weight for p, pos in zip(pallets, positions)
        )
        weighted_y = sum(
            (pos[1] + p.width / 2) * p.max_weight for p, pos in zip(pallets, positions)
        )
    ...
    def _crossover(
        self, p1: List[PalletSpec], p2: List[PalletSpec]
    ) -> List[PalletSpec]:
    ...
    def _mutate(self, individual: List[PalletSpec]) -> List[PalletSpec]:
        ...
        valid_pallets = [
            p
            for p in self.candidate_pallets
            if p.length <= individual[idx].length * 1.2
        ]
        if not valid_pallets:
            return individual
...
class TowerPackingAlgorithm:
    ...
    def __init__(
        ...
        cargo_type: str = "pallet",  # pallet/bulk, 托盘/散货区分标志
    ...
    def _init_business_rules(
        self, transport_type: str, cargo_type: str, container_type: str
    ):
        """根据运输场景初始化业务规则"""
        # 高度限制规则选择
        if transport_type == "sea":
            if cargo_type == "pallet":
                ...
                if container_type == "high":
                    self.max_height = (
                        BusinessRules.MAX_GOODS_ALTITUDE_OF_PLATOON_TOWAGE_OF_SEA_TRANSPORT_IN_HIGH_CONTAINER
                    )
            else:
                self.max_height = (
                    BusinessRules.MAX_GOODS_ALTITUDE_OF_BULK_CARGO_OF_SEA_TRANSPORT_IN_GENERAL_CONTAINER
                )
                if container_type == "high":
                    self.max_height = (
                        BusinessRules.MAX_GOODS_ALTITUDE_OF_BULK_CARGO_OF_SEA_TRANSPORT_IN_HIGH_CONTAINER
                    )
        elif transport_type == "air":
            self.max_height = (
                BusinessRules.MAX_GOODS_ALTITUDE_OF_PLATOON_TOWAGE_OF_AIR_FREIGHT
            )
        else:
    ...
    def _is_valid_dimension(self, l: int, w: int, h: int) -> bool:
        """检查尺寸是否符合业务规则（增加容差处理）"""
        eff_length = (
            self.pallet.length - 2 * self.edge_gap + BusinessRules.SIZE_TOLERANCE
        )
    ...
    def _can_place_product(self, product: ProductsSpec, dim: tuple) -> bool:
        ...
        if self.positions:
            ...
            overlap_area = min(dim[0], last["dimensions"][0]) * min(
                dim[1], last["dimensions"][1]
            )
            min_area = (
                product.length * product.width * BusinessRules.SUPPORT_AREA_MIN_LIMIT
            )
    ...
    def _calculate_priority(self, product: ProductsSpec) -> float:
        """计算货物优先级(易碎性 > 重量 > 高度)"""
        # 标准化各项指标（0-1范围）
        fragility_norm = product.fragility / BusinessRules.MAX_FRAGILITY
        weight_norm = (
            product.weight / max(p.weight for p in self.products)
            if self.products
            else 0
        )
        size_norm = (
            (product.length * product.width * product.height)
            / max(p.length * p.width * p.height for p in self.products)
            if self.products
            else 0
        )
        ...
        if self.transport_type == "air":
            # 空运更关注重量和易碎性
            weights = (0.5, 0.3, 0.2)  # (易碎性, 重量, 尺寸)
        else:
            # 海运更关注尺寸和稳定性
            weights = (0.3, 0.2, 0.5)

        return (
            weights[0] * fragility_norm
            + weights[1] * weight_norm
            + weights[2] * size_norm
        )
    ...
    def _calculate_utilization(self) -> float:
        ...
        available = (
            (self.pallet.length - 2 * self.edge_gap)
            * (self.pallet.width - 2 * self.edge_gap)
            * self.max_height
        )
    ...
    def _check_stability(self) -> dict:  # 存疑
        ...
        # 计算重心坐标
        weighted_x = sum(
            (p["position"][0] + p["dimensions"][0] / 2) * p["product"].weight
            for p in self.positions
        )
        weighted_y = sum(
            (p["position"][1] + p["dimensions"][1] / 2) * p["product"].weight
            for p in self.positions
        )
        center_x = weighted_x / total_weight
        center_y = weighted_y / total_weight
        ...
        return {
            ...
            "threshold": (
                BusinessRules.SEA_OFFSET_LIMIT
                if self.transport_type == "sea"
                else BusinessRules.AIR_OFFSET_LIMIT
            ),
        }
...
class BinPacking2D:
    ...
    def __init__(
        ...
        global_offset: Tuple[float, float] = (0, 0),
    ):
        """
        参数：
        ...
        if container_size[0] <= 0 or container_size[1] <= 0:
            raise ValueError("容器尺寸必须为正数")
        self.width, self.height = container_size
        self.points = [(0, 0, self.width, self.height)]
    ...
    def greedy_packing(self, items: List[dict]) -> List[dict]:
        """贪婪装载算法"""
        sorted_items = sorted(
            items, key=lambda x: -(x["dimensions"][0] * x["dimensions"][1])
        )
        ...
        for item in sorted_items:
            placed = False
            for point in sorted(
                self.active_points, key=lambda p: -p["width"] * p["height"]
            ):
    ...
    def _can_place(self, item, point) -> bool:
        ...
        return (
            item_w <= point["width"] and item_h <= point["height"] and point["active"]
        )
    ...
    def _place_item(self, item, point):
        ...
        # 生成后方剩余区域
        remaining_height = point["height"] - (
            item["dimensions"][1] + self.gap_longitudinal
        )
    ...
    def _calculate_utilization(self, solution) -> float:
        ...
        effective_width = (
            self.width
            - BusinessRules.PALLET_GAP_CONTAINER["left"]
            - BusinessRules.PALLET_GAP_CONTAINER["right"]
        )
        effective_height = (
            self.height
            - BusinessRules.PALLET_GAP_CONTAINER["front"]
            - BusinessRules.PALLET_GAP_CONTAINER["back"]
        )
...
class HybridOptimizer:
    ...
    def __init__(
        ...
        if not isinstance(container, ContainerSpec):
            raise ValueError(
                f"无效的集装箱规格类型，期望ContainerSpec，实际得到{type(container)}"
            )
        ...
        if not all(isinstance(p, ProductsSpec) for p in products):
            invalid_types = {
                type(p) for p in products if not isinstance(p, ProductsSpec)
            }
    ...
    def optimize(self) -> Dict:
        """执行混合优化流程（GA → PSO → SA）"""
        # 首先验证所有产品是否至少有一种旋转方向可以放入容器
        if not self.use_pallet:
            for product in self.products:
                can_fit = False
                for rot in product.allowed_rotations:
                    if (
                        rot[0] <= self.container.length
                        and rot[1] <= self.container.width
                        and rot[2] <= self.container.height
                    ):
                        can_fit = True
                        break
    ...
    def _validate_constraints(self, solution: Solution) -> bool:
        ...
        if self.use_pallet:
            ...
            return checker.check_all(
                products, pallets, product_positions, pallet_positions
            )
        else:
            return checker.check_all(solution.items, [], solution.positions, [])

    def _check_pallet_gaps(
        self, pallets: List[PalletSpec], positions: List[tuple]
    ) -> bool:
        """检查托盘间隙约束"""
        for pallet, (x, y, _) in zip(pallets, positions):
            # 检查与集装箱边界的间隙
            if (
                x < BusinessRules.PALLET_GAP_CONTAINER["left"]
                or y < BusinessRules.PALLET_GAP_CONTAINER["front"]
                or self.container.length - (x + pallet.length)
                < BusinessRules.PALLET_GAP_CONTAINER["back"]
                or self.container.width - (y + pallet.width)
                < BusinessRules.PALLET_GAP_CONTAINER["right"]
            ):
                return False
        return True

    def _check_fragile_stack(
        self, products: List[ProductsSpec], positions: List[tuple]
    ) -> bool:
        """检查易碎品堆叠约束"""
...
class ACO:
    ...
    def _accept(self, current_energy: float, new_energy: float, temp: float) -> bool:
        """模拟退火接受准则"""
        if new_energy < current_energy:
            return True  # 总是接受更好的解

        if temp <= 0:
            return False  # 零温或负温时不接受差解

        # 计算接受概率
        delta = current_energy - new_energy  # 负值，因为new_energy > current_energy
    ...
    def generate_solutions(
        self, container: ContainerSpec, products: List[ProductsSpec]
    ) -> List[Solution]:
        solutions = []
    ...
    def _construct_solution(
        ...
        """构造解决方案"""
        # 输入验证
        if not products or not container:
            return Solution(items=[], positions=[], placements=[])

        # 按体积降序排列货物（无托盘时更注重空间利用）
        remaining_products = sorted(products, key=lambda x: -x.volume)

        # 首先过滤掉无论如何旋转都无法放入容器的产品
        remaining_products = [
            p
            for p in remaining_products
            if any(
                rot[0] <= container.length
                and rot[1] <= container.width
                and rot[2] <= container.height
                for rot in p.allowed_rotations
            )
        ]
        ...
        while remaining_products and used_volume < container.volume:
            ...
            for i, product in enumerate(remaining_products):
                ...
                valid_rotations = [
                    ...
                    for rot in product.allowed_rotations
                    if (
                        current_position[0] + rot[0] <= container.length
                        and current_position[1] + rot[1] <= container.width
                        and current_position[2] + rot[2] <= container.height
                    )
                ]
                ...
                # 再次验证尺寸
                if (
                    best_rotation[0] > container.length
                    or best_rotation[1] > container.width
                    or best_rotation[2] > container.height
                ):
                    continue
                ...
                if (
                    current_position[0]
                    + (best_rotation[0] if remaining_products else 0)
                    > container.length
                ):
                    ...
                    if (
                        current_position[1]
                        + (best_rotation[1] if remaining_products else 0)
                        > container.width
                    ):
        ...
        positions = [
            (p.position[0], p.position[1], p.position[2], p.dimensions)
            for p in placements
        ]
        ...
        return Solution(
            ...
            weight_utilization=sum(p.weight for p in items) / container.max_weight,
            stability_score=self._calculate_stability(placements, container),
        )

    def _select_next_product(
        self, products: List[ProductsSpec], current_pos: List[int]
    ) -> ProductsSpec:
        """概率选择下一个货物"""
        if not products:
            return None
        ...
        # 计算选择概率
        probabilities = [
            (ph**self.alpha) * (h**self.beta) for ph, h in zip(pheromone, heuristic)
        ]
    ...
    def _select_best_rotation(
        self, product: ProductsSpec, pos: List[int], container: ContainerSpec
    ) -> tuple:
        """计算最佳旋转方向"""
        valid_rotations = []
        for rot in product.allowed_rotations:
            # 检查是否超出集装箱边界
            if (
                pos[0] + rot[0] <= container.length
                and pos[1] + rot[1] <= container.width
                and pos[2] + rot[2] <= container.height
            ):
    ...
    def _update_pheromone(self, solution: Solution):
        ...
        # 确保质量计算有效
        if solution.placements and hasattr(solution, "volume_utilization"):
            quality = solution.volume_utilization
            # 确保质量是正值
            quality = max(0.1, quality)  # 最小0.1保证信息素增加

            for placement in solution.placements:
                if hasattr(placement, "product") and hasattr(placement.product, "id"):
                    product_id = placement.product.id
                    self.pheromone[product_id] += quality

    def _calculate_stability(
        self, placements: List[Placement], container: ContainerSpec
    ) -> float:
        """计算稳定性得分"""
        if not placements:
            return 0.0

        total_weight = sum(p.product.weight for p in placements)
        if total_weight <= 0:
            return 0.0

        # 计算重心坐标
        weighted_x = sum(
            (p.position[0] + p.dimensions[0] / 2) * p.product.weight for p in placements
        )
        weighted_y = sum(
            (p.position[1] + p.dimensions[1] / 2) * p.product.weight for p in placements
        )
...
class PSO:
    ...
    def optimize(
        self, initial_solutions: List[Solution], container: ContainerSpec
    ) -> Solution:
        # 初始化种群
        ...
        for iteration in range(AlgorithmParams.PSO_MAX_ITER):
            ...
            for particle in self.population:
                ...
                if (
                    ...
                    or particle.best_fitness > self.global_best.best_fitness
                ):
    ...
    def _initialize_population(
        ...
        for i in range(self.pop_size):
            ...
            if len(particle.position) == 0:
                particle.position = self._encode_solution(
                    valid_solutions[0]
                )  # 回退到第一个有效解

            particle.velocity = [
                random.uniform(-1, 1) for _ in range(len(particle.position))
            ]
            particle.best_position = deepcopy(particle.position)
        ...
        # 初始化全局最优
        if self.population:
            self.global_best = deepcopy(
                max(self.population, key=lambda x: x.best_fitness)
            )
        else:
            # 提供默认粒子
            self.global_best = Particle()
            self.global_best.best_position = self._encode_solution(
                self._generate_default_solution(container)
            )
    ...
    def _generate_default_solution(self, container: ContainerSpec) -> Solution:
        ...
        return Solution(items=items, positions=positions, placements=placements)
        ...
        for placement in solution.placements:
            ...
            encoded.extend(
                [
                    ...
                    float(
                        placement.product.allowed_rotations.index(placement.dimensions)
                    ),
    ...
    def _decode_solution(
        self, position: list, products: List[ProductsSpec]
    ) -> Solution:
        """
        从位置向量解码解决方案
        参数:
        ...
        while i + 4 < len(position):
            try:
                ...
                # 安全获取位置坐标
                x = max(
                    0, min(float(position[i + 2]), self.container.length - rotation[0])
                )
                y = max(
                    0, min(float(position[i + 3]), self.container.width - rotation[1])
                )
                z = max(
                    0, min(float(position[i + 4]), self.container.height - rotation[2])
                )

                placements.append(
                    Placement(product=product, position=(x, y, z), dimensions=rotation)
                )
        ...
        return Solution(
            ...
            placements=placements,
            volume_utilization=(
                sum(p.volume for p in items) / self.container.volume if items else 0
            ),
        )
    ...
    def _adjust_inertia(self, iteration: int):
        ...
        self.inertia = start - (start - end) * (
            iteration / AlgorithmParams.PSO_MAX_ITER
        )

    def _update_velocity(self, particle: Particle):
        """更新粒子速度"""
        if (
            not hasattr(self.global_best, "best_position")
            or len(self.global_best.best_position) == 0
        ):
            return  # 跳过无效的全局最优解
        ...
        for i in range(len(particle.velocity)):
            ...
            social = (
                self.social_weight
                * r2
                * (self.global_best.best_position[i] - particle.position[i])
            )

            # 更新速度
            particle.velocity[i] = (
                self.inertia * particle.velocity[i] + cognitive + social
            )

            # 限制速度
            if abs(particle.velocity[i]) > self.max_velocity:
                particle.velocity[i] = self.max_velocity * (
                    1 if particle.velocity[i] > 0 else -1
                )
    ...
    def _repair_solution(
        self, solution: Solution, container: ContainerSpec
    ) -> Solution:
        """修复无效的解决方案"""
        ...
        for placement in solution.placements:
            # 检查是否重复装载同一货物
            if (
                placement.product.id in used_products
            ):  # 同一货物装载多次如何处理？用id是否不妥？
                continue

            # 检查是否超出集装箱边界
            if (
                placement.position[0] + placement.dimensions[0] <= container.length
                and placement.position[1] + placement.dimensions[1] <= container.width
                and placement.position[2] + placement.dimensions[2] <= container.height
            ):
        ...
        return Solution(items=items, positions=positions, placements=valid_placements)
    ...
    def _evaluate(self, particle: Particle, container: ContainerSpec):
        ...
        # 计算托盘总承重和体积（如果需要）
        pallet_max_weight = (
            np.sum([p.max_weight for p in pallets]) if pallets else container.max_weight
        )

        # 计算利用率
        particle.solution.volume_utilization = min(total_volume / container.volume, 1.0)
        particle.solution.weight_utilization = (
            min(total_weight / pallet_max_weight, 1.0) if pallet_max_weight > 0 else 0.0
        )

        # 计算稳定性（略）
        particle.solution.stability_score = self._calculate_stability(
            particle.solution, container
        )
    ...
    def _calculate_overlap_penalty(self, solution: Solution) -> float:
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                overlap = not (
                    max_corners[i, 0] <= positions[j, 0]
                    or positions[i, 0] >= max_corners[j, 0]
                    or max_corners[i, 1] <= positions[j, 1]
                    or positions[i, 1] >= max_corners[j, 1]
                    or max_corners[i, 2] <= positions[j, 2]
                    or positions[i, 2] >= max_corners[j, 2]
                )
                ...
                if overlap:
                    ...
                    overlap_vol = (
                        max(
                            0,
                            min(max_corners[i, 0], max_corners[j, 0])
                            - max(positions[i, 0], positions[j, 0]),
                        )
                        * max(
                            0,
                            min(max_corners[i, 1], max_corners[j, 1])
                            - max(positions[i, 1], positions[j, 1]),
                        )
                        * max(
                            0,
                            min(max_corners[i, 2], max_corners[j, 2])
                            - max(positions[i, 2], positions[j, 2]),
                        )
                    )
...
class SA:
    ...
    def optimize(
        self, initial_solution: Solution, container: ContainerSpec
    ) -> Solution:
    ...
    def _perturb(self, solution: Solution, container: ContainerSpec) -> Solution:
        ...
        for placement in new_solution.placements:
            if not isinstance(placement.product, ProductsSpec):
                raise TypeError(f"期望ProductsSpec，得到{type(placement.product)}")

        # 随机选择扰动方式
        mutation_type = random.choice(["swap", "rotate", "move", "add", "remove"])

        if mutation_type == "swap" and len(new_solution.placements) >= 2:
            # 交换两个货物的位置
            i, j = random.sample(range(len(new_solution.placements)), 2)
            new_solution.placements[i], new_solution.placements[j] = (
                new_solution.placements[j],
                new_solution.placements[i],
            )
            ...
            product = placement.product  # 确保 product 是 ProductsSpec 对象

            if isinstance(product, ProductsSpec):
                new_rotation = random.choice(product.allowed_rotations)
                new_solution.placements[idx].dimensions = new_rotation
            else:
                # 如果 product 不是 ProductsSpec 对象，跳过旋转
                pass
            ...
            mutation_type == "add"
            and len(new_solution.placements) < len(solution.placements) * 1.5
        ):
            ...
            available_products = [
                p
                for p in self.products
                if isinstance(p, ProductsSpec) and p.id not in loaded_ids
            ]

            if available_products:
                product = random.choice(available_products)

                if not isinstance(product, ProductsSpec):
                    return new_solution

                # 确保有有效的旋转方向
                if (
                    not hasattr(product, "allowed_rotations")
                    or not product.allowed_rotations
                ):
                    rotation = (
                        getattr(product, "length", 100),
                        getattr(product, "width", 100),
                        getattr(product, "height", 100),
                    )
                else:
                ...
                for _ in range(10):
                    try:
                        ...
                        if self._is_valid_position(
                            new_solution.placements, (x, y, z), rotation
                        ):
                            new_solution.placements.append(
                                Placement(
    ...
    def _is_valid_placement(
        self, position: tuple, dimensions: tuple, container: ContainerSpec
    ) -> bool:
        """检查放置是否有效"""
        return (
            position[0] + dimensions[0] <= container.length
            and position[1] + dimensions[1] <= container.width
            and position[2] + dimensions[2] <= container.height
        )

    def _is_valid_position(
        self, placements: List[Placement], position: tuple, dimensions: tuple
    ) -> bool:
        """检查新位置是否有效"""
        # 检查容器边界
        if (
            ...
            or position[1] < 0
            or position[2] < 0
            or position[0] + dimensions[0] > self.container.length
            or position[1] + dimensions[1] > self.container.width
            or position[2] + dimensions[2] > self.container.height
        ):
            return False

        # 检查门区预留
        if (
            position[0] < self.container.door_reserve
            or position[1] < self.container.door_reserve
        ):
            return False
        ...
        for placement in placements:
            ...
            # 检查是否有重叠
            if not (
                new_max[0] <= existing_min[0]
                or new_min[0] >= existing_max[0]
                or new_max[1] <= existing_min[1]
                or new_min[1] >= existing_max[1]
                or new_max[2] <= existing_min[2]
                or new_min[2] >= existing_max[2]
            ):
                return False
    ...
    def _energy(self, solution: Solution, container: ContainerSpec) -> float:
        ...
        total_weight = sum(p.product.weight for p in solution.placements)
        weight_utilization = (
            total_weight / container.max_weight if container.max_weight > 0 else 0.0
        )
        ...
        return (
            0.7 * unused_volume + 0.2 * instability + 0.1 * (1.0 - weight_utilization)
        )
    ...
    def _accept(self, current_energy: float, new_energy: float) -> bool:
        ...
        # 计算接受概率
        delta = new_energy - current_energy  # 正值，因为 new_energy > current_energy
        acceptance_prob = math.exp(-delta / (self.k * self.temp))
...
class FlowerStackOptimizer:
    ...
    def __init__(self, container: ContainerSpec, products: List[ProductsSpec]):
        ...
        self.products = sorted(
            products, key=lambda x: x.fragility, reverse=True
        )  # 存疑
        self.layers = []
        self.current_z = 0
    ...
    def _build_layer(self):  # 存疑
        ...
        while remaining_width > 0 and self.products:
            ...
            for item in selected:
                pos = (current_x, 0, self.current_z)
                dimensions = (item.width, item.length, item.height)
                layer.append(
                    Placement(product=item, position=pos, dimensions=dimensions)
                )
                current_x += item.width + BusinessRules.PALLET_GAP["lateral"]
                remaining_width -= item.width
                self.products.remove(item)

        self.current_z += max(item.dimensions[2] for item in layer) if layer else 0
        return layer
    ...
    def _select_by_lp(self, max_width):  # 存疑
        ...
        # 目标函数：最大化重量稳定性
        prob += sum(
            vars[i] * (self.products[i].weight * self.products[i].width)
            for i in range(len(vars))
        )

        # 约束条件
        prob += (
            sum(vars[i] * self.products[i].width for i in range(len(vars))) <= max_width
        )
    ...
    def _adjust_centroid(self, layer):
        ...
        total_weight = sum(item.product.weight for item in layer)
        if total_weight <= 0:
            return

        centroid_x = (
            sum(item.position[0] * item.product.weight for item in layer) / total_weight
        )

        # 获取前一层的跨度范围
        prev_layer = self.layers[-1]
        prev_min_x = min(item.position[0] for item in prev_layer)
        prev_max_x = max(item.position[0] + item.dimensions[0] for item in prev_layer)

        # 计算需要调整的偏移量
        if centroid_x < prev_min_x:
            delta = prev_min_x - centroid_x
        elif centroid_x > prev_max_x:
            delta = prev_max_x - centroid_x
        else:
            return  # 已经在支撑范围内，不需要调整

        # 调整所有物品的位置
        for item in layer:
            new_x = item.position[0] + delta
            item.position = (new_x, item.position[1], item.position[2])
...
class NSGAII:
    ...
    def _mutate(self, solution: Solution) -> Solution:
        ...
        if len(mutated.items) > 1:
            ...
            mutated.positions[i], mutated.positions[j] = (
                mutated.positions[j],
                mutated.positions[i],
            )
        return mutated
...
class ParallelEvaluator:
    ...
    def _calculate_stability(self, solution: Solution) -> float:
        ...
        total_weight = sum(p.weight for p in solution.items)
        weighted_x = sum(
            (pos[0] + dim[0] / 2) * p.weight
            for p, pos, dim in zip(
                solution.items,
                solution.positions,
                [p.allowed_rotations[0] for p in solution.items],
            )
        )
        weighted_y = sum(
            (pos[1] + dim[1] / 2) * p.weight
            for p, pos, dim in zip(
                solution.items,
                solution.positions,
                [p.allowed_rotations[0] for p in solution.items],
